...
// We're using Postcss on account of trying out Tailwind .... yet more learning
...
// 		.pipe(postcss([tailwindcss,cssImport, cssvars, nested]))
 ...
 *    1. Gets the source scss file
 *    2. Compiles Sass to CSS
 *	  3. Uses PostCSS to import Tailwind styles and interpret Tailwind 			syntax
 *    3. Writes Sourcemaps for it
 *    4. Autoprefixes it and generates style.css
 *    5. Renames the CSS file with suffix .min.css
 *    6. Minifies the CSS file and generates style.min.css
...
gulp.task(
	'dev',
	gulp.parallel('dev-styles', 'vendorsJS', 'customJS', 'images', browsersync, () => {
		gulp.watch(config.watchPhp, reload); // Reload on PHP file changes.
		gulp.watch(config.watchStyles, gulp.parallel('dev-styles')); // Reload on SCSS file changes.
		gulp.watch(config.watchJsVendor, gulp.series('vendorsJS', reload)); // Reload on vendorsJS file changes.
		gulp.watch(config.watchJsCustom, gulp.series('customJS', reload)); // Reload on customJS file changes.
		gulp.watch(config.imgSRC, gulp.series('images', reload)); // Reload on customJS file changes.
	})
